            ...
            content = {
                Column(modifier = Modifier.padding(it)) {
                    when (netShareState) {
                        ...
                        is NetShareInfoState -> NetShareGrid(netShareState.netShares, filesViewModel, onShareClick)
...
@Composable
internal fun NetShareGrid(netShareItems: List<NetShareInfo1>, fileViewModel: FilesViewModel, onShareClick: (String) -> Unit) {
    LazyVerticalGrid(
        columns = Adaptive(120.dp),
        contentPadding = PaddingValues(1.dp),
    ) {
        items(netShareItems) { item ->
            NetSharePlate(netShare = item, onShareClick, fileViewModel)
        }
    }
}
